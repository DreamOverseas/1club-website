    ...
    try {
      ...
      if (userResponse.ok && userData.data && userData.data.length > 0) {
        ...
        const updateResponse = await fetch(
          `${endpoint}/api/one-club-memberships/${documentId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    <Container className="my-4">
      ...
      <Row>
        {filteredProducts.map((product) => {
          ...
          const iconUrl = Icon?.url ? `${endpoint}${Icon.url}` : "";
          ...
          <Modal.Body>
            ...
            {(() => {
              ...
              return (
                <>
                  <p>
                    现金：{redeemProduct.Price - currDeduction} → 兑换后余额{" "}
                    <b>{cookiePoint - redeemProduct.Price + currDeduction}</b>
                  </p>
                  <p>
          ...
          <Modal.Footer>
            {(() => {
              ...
              const cookieDiscountPoint =
                JSON.parse(Cookies.get("user")).discount_point || 0;
